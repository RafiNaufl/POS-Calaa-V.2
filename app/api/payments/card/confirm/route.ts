...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only admin can confirm CARD payments
    if (user.role !== 'ADMIN') {
      return NextResponse.json(
    ...
    // Verify transaction is CARD and is pending
    if (transaction.paymentMethod !== 'CARD') {
      return NextResponse.json(
        { error: 'Bad Request', message: 'Transaksi ini bukan pembayaran Kartu.' },
        { status: 400 }
      )
    }
    ...
    // Send WhatsApp receipt automatically when CARD confirmed
    try {
      if (transaction.customerPhone) {
        ...
        if (fullTransaction) {
          ...
          if (phoneValidation.isValid) {
            ...
            if (result.success) {
              console.log(`[CARD] WhatsApp receipt sent for transaction ${transactionId}`)
            } else {
              console.warn(`[CARD] Failed to send WhatsApp receipt for transaction ${transactionId}: ${result.error}`)
            }
          } else {
            console.warn(`[CARD] Invalid phone number for WhatsApp receipt: ${fullTransaction.customerPhone}`)
          }
        }
      }
    } catch (waErr) {
      console.warn(`[CARD] WhatsApp receipt sending error for transaction ${transactionId}:`, waErr)
    }
