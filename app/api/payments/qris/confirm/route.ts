...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only admin can confirm QRIS payments
    if (user.role !== 'ADMIN') {
      return NextResponse.json(
    ...
    // Verify transaction is QRIS and is pending
    if (transaction.paymentMethod !== 'QRIS') {
      return NextResponse.json(
        { error: 'Bad Request', message: 'Transaksi ini bukan pembayaran QRIS.' },
        { status: 400 }
      )
    }
    ...
    // Send WhatsApp receipt automatically when QRIS confirmed
    try {
      if (transaction.customerPhone) {
        // Refetch full transaction including member and user for better receipt formatting
        ...
        if (fullTransaction) {
          ...
          if (phoneValidation.isValid) {
            ...
            if (result.success) {
              console.log(`[QRIS] WhatsApp receipt sent for transaction ${transactionId}`)
            } else {
              console.warn(`[QRIS] Failed to send WhatsApp receipt for transaction ${transactionId}: ${result.error}`)
            }
          } else {
            console.warn(`[QRIS] Invalid phone number for WhatsApp receipt: ${fullTransaction.customerPhone}`)
          }
        }
      }
    } catch (waErr) {
      console.warn(`[QRIS] WhatsApp receipt sending error for transaction ${transactionId}:`, waErr)
    }
